...
overwrites the previously-good HTML with half-updated data:
  ...
  2. daily_nav.py            - recompute the full since-inception NAV curve (paper strategy)
  3. build_fund_nav_json.py  - repack it for the chart
  4. fetch_benchmark.py      - refresh the Nifty Midcap 150 benchmark series
  5. build_actual_nav.py     - mark the real funded portfolio to market
  6. build_ladder_data.py    - rebuild today's ladder/dropped/added/screened/weights
  7. build_live_ladder.py    - rebuild Live_Portfolio_Monitor.html
...
STEPS = [
    ...
    ("Mark actual portfolio to market", [sys.executable, "build_actual_nav.py"]),
