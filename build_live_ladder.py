...
<div class="wrap">
  ...
  <div class="panel" id="actualPanel" style="display:none">
    ...
    <div class="legend-note" style="margin-top:8px">
      <span><span class="dot" style="background:var(--bad)"></span>Highlighted / "At risk" = ranked below the buffer today — would be dropped if the rebalance happened now</span>
      <span>Click a column header to sort · shift-click to add a secondary sort key</span>
    </div>
  </div>
...
if (DATA.actual) {
  ...
  function renderHoldingsTable() {
    ...
    rows.forEach(r => {
      ...
      const atRisk = dropTkrs.has(r.tkr);
      const tkrCell = atRisk
        ? `<td>${r.tkr} <span class="badge badge-at-risk">At risk</span></td>`
        : `<td>${r.tkr}</td>`;
      hh += `<tr class="${atRisk ? 'row-at-risk' : ''}"><td>${r.idx}</td>${tkrCell}<td>${r.qty}</td>` +
