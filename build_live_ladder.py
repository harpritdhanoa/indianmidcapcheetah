...
<div class="wrap">
  ...
  <div class="panel">
    ...
    <div class="chart-toolbar draw-toolbar">
      <div class="tool-group">
        <button class="tool-btn active" data-tool="navigate" onclick="setTool('navigate')">Navigate</button>
        <button class="tool-btn" data-tool="hline" onclick="setTool('hline')">Horizontal line</button>
        <button class="tool-btn" data-tool="arrow" onclick="setTool('arrow')">Arrow</button>
      </div>
      <div class="tool-group">
        <label>Label: <select id="labelModeSelect">
          <option value="both" selected>% + CAGR</option>
          <option value="pct">% only</option>
          <option value="cagr">CAGR only</option>
        </select></label>
        <button onclick="clearAnnotations()">Clear drawings</button>
        <button onclick="resetZoom('navChart')">Reset zoom</button>
      </div>
    </div>
    <div class="chart-wrap"><canvas id="navChart"></canvas></div>
    <div class="callout">Net of the same 0.25% one-way transaction cost and semi-annual rebalance/buffer/cap rules as the live selection above. Max drawdown here is measured on the actual daily NAV path, not just at rebalance dates — it will read worse (and more honestly) than any figure computed only at the ~21 semi-annual checkpoints. Navigate mode: scroll/pinch to zoom (both axes), click and drag to pan. Horizontal line: click anywhere to mark a NAV level. Arrow: click one date on the curve and drag to another to measure the return between them (snaps to the actual NAV series, not a free point) — CAGR is only shown for intervals of 90+ days, since annualizing a short move produces a technically-correct but misleading number; shorter arrows show % only.</div>
  </div>
...
// --- Drawing tools: horizontal lines + arrows (return/CAGR measurement) ---
let currentTool = 'navigate';
let hlines = [];       // [{ y: navValue }]
let arrows = [];       // [{ idx1, idx2 }] indices into FUND.dates/FUND.nav, idx1 < idx2
let previewArrow = null;
let dragStartIdx = null;
let labelMode = 'both';

// CAGR only means something once you're annualizing over a reasonably long
// stretch - compounding a two-week wiggle out to "per year" produces numbers
// like -80% that are technically the correct extrapolation but not a useful
// or intuitive read of a short move. Below MIN_CAGR_DAYS, only % is shown.
const MIN_CAGR_DAYS = 90;

function computeMeasure(idx1, idx2) {
  const val1 = FUND.nav[idx1], val2 = FUND.nav[idx2];
  const date1 = FUND.dates[idx1], date2 = FUND.dates[idx2];
  const days = (new Date(date2) - new Date(date1)) / 86400000;
  const years = days / 365.25;
  const pct = (val2 / val1 - 1) * 100;
  const cagr = days >= MIN_CAGR_DAYS ? (Math.pow(val2 / val1, 1 / years) - 1) * 100 : null;
  return { val1, val2, date1, date2, days, pct, cagr };
}

function measureLabel(m) {
  const sign = (v) => (v >= 0 ? '+' : '');
  const pctTxt = `${sign(m.pct)}${m.pct.toFixed(1)}%  (${Math.round(m.days)}d)`;
  const cagrTxt = m.cagr != null
    ? `CAGR ${sign(m.cagr)}${m.cagr.toFixed(1)}%`
    : `CAGR n/a (<${MIN_CAGR_DAYS}d)`;
  if (labelMode === 'pct') return pctTxt;
  if (labelMode === 'cagr') return cagrTxt;
  return `${pctTxt}  |  ${cagrTxt}`;
}

// Category-scale-safe pixel<->index conversion: uses two known reference
// points (index 0 and index n-1) rather than relying on getValueForPixel,
// which behaves inconsistently across Chart.js scale types after zoom/pan.
function xPixelToIndex(chart, px) {
  const scale = chart.scales.x;
  const n = FUND.nav.length;
  const leftPx = scale.getPixelForValue(0);
  const rightPx = scale.getPixelForValue(n - 1);
  const frac = (px - leftPx) / (rightPx - leftPx);
  return Math.max(0, Math.min(n - 1, Math.round(frac * (n - 1))));
}

const navAnnotationPlugin = {
  id: 'navAnnotations',
  afterDraw(chart) {
    ...
    hlines.forEach((h) => {
      const yPx = scales.y.getPixelForValue(h.y);
      ...
      ctx.stroke();
      ctx.setLineDash([]);
      const pct = (h.y / FUND.nav[0] - 1) * 100;
      const label = `${h.y.toFixed(2)}x  (${pct >= 0 ? '+' : ''}${pct.toFixed(1)}% vs inception)`;
      ctx.font = '11px -apple-system, BlinkMacSystemFont, sans-serif';
      const textW = ctx.measureText(label).width;
      const boxX = chartArea.right - textW - 12;
      ctx.fillStyle = 'rgba(23,26,35,0.9)';
      ctx.fillRect(boxX - 4, yPx - 16, textW + 8, 16);
      ctx.fillStyle = '#e8b84b';
      ctx.fillText(label, boxX, yPx - 4);
    });

    function drawArrow(idx1, idx2, color, showLabel) {
      const x1 = scales.x.getPixelForValue(idx1), y1 = scales.y.getPixelForValue(FUND.nav[idx1]);
      const x2 = scales.x.getPixelForValue(idx2), y2 = scales.y.getPixelForValue(FUND.nav[idx2]);
      ...
      if (showLabel && idx1 !== idx2) {
        const m = computeMeasure(idx1, idx2);
        const label = measureLabel(m);
        const midX = (x1 + x2) / 2, midY = (y1 + y2) / 2;
        ctx.font = '11px -apple-system, BlinkMacSystemFont, sans-serif';
        const textW = ctx.measureText(label).width;
        ctx.fillStyle = 'rgba(23,26,35,0.92)';
        ctx.fillRect(midX - textW / 2 - 5, midY - 22, textW + 10, 16);
        ctx.fillStyle = color;
        ctx.fillText(label, midX - textW / 2, midY - 10);
      }
    }

    arrows.forEach((a) => drawArrow(a.idx1, a.idx2, '#3ecf8e', true));
    if (previewArrow) drawArrow(previewArrow.idx1, previewArrow.idx2, 'rgba(91,141,239,0.9)', true);

    ctx.restore();
  },
};

charts['navChart'] = new Chart(document.getElementById('navChart'), {
  type: 'line',
  data: {
    labels: FUND.dates,
    datasets: [{
  ...
  options: {
    ...
    scales: {
      x: {
        grid: { color: '#2a2f3f' },
        ticks: { color: '#9099ac', maxTicksLimit: 12 },
      },
      y: {
        grid: { color: '#2a2f3f' },
        ticks: { color: '#9099ac', callback: (v) => v.toFixed(1) + 'x' },
      },
    },
    plugins: {
...
function setTool(tool) {
  ...
  const chart = charts['navChart'];
  const nav = tool === 'navigate';
  chart.options.plugins.zoom.pan.enabled = nav;
  chart.options.plugins.zoom.zoom.wheel.enabled = nav;
  chart.options.plugins.zoom.zoom.pinch.enabled = nav;
  chart.canvas.style.cursor = nav ? 'default' : 'crosshair';
  dragStartIdx = null;
  previewArrow = null;
  chart.update('none');
}
...
const navCanvas = charts['navChart'].canvas;
...
navCanvas.addEventListener('click', (evt) => {
  if (currentTool !== 'hline') return;
  const p = canvasPoint(evt);
  const chart = charts['navChart'];
  const val = chart.scales.y.getValueForPixel(p.y);
  hlines.push({ y: val });
  chart.update('none');
});

navCanvas.addEventListener('mousedown', (evt) => {
  if (currentTool !== 'arrow') return;
  const chart = charts['navChart'];
  const p = canvasPoint(evt);
  const idx = xPixelToIndex(chart, p.x);
  dragStartIdx = idx;
  previewArrow = { idx1: idx, idx2: idx };
  chart.draw();
});

navCanvas.addEventListener('mousemove', (evt) => {
  if (currentTool !== 'arrow' || dragStartIdx === null) return;
  const chart = charts['navChart'];
  const p = canvasPoint(evt);
  const idx = xPixelToIndex(chart, p.x);
  previewArrow = { idx1: Math.min(dragStartIdx, idx), idx2: Math.max(dragStartIdx, idx) };
  chart.draw();
});

navCanvas.addEventListener('mouseup', (evt) => {
  if (currentTool !== 'arrow' || dragStartIdx === null) return;
  const chart = charts['navChart'];
  const p = canvasPoint(evt);
  const idx = xPixelToIndex(chart, p.x);
  if (idx !== dragStartIdx) {
    arrows.push({ idx1: Math.min(dragStartIdx, idx), idx2: Math.max(dragStartIdx, idx) });
  }
  dragStartIdx = null;
  previewArrow = null;
  chart.update('none');
...
navCanvas.addEventListener('mouseleave', () => {
  if (currentTool === 'arrow' && dragStartIdx !== null) {
    dragStartIdx = null;
    previewArrow = null;
    charts['navChart'].update('none');
  }
